<?php 
	...
	class NuevoLibroModel extends Model
	{
		private $idAutor;
		private $idTexto;
		...
		public function InsertarColeccion($datos){
			...
			$query = "INSERT INTO coleccion(id_texto, id_genero, id_categoria) VALUES('$this->idTexto','6','1')";
			$this->db->query($query);
			if($this->db->affected_rows < 0){
				echo "-ERROR: No se pudo insertar Coleccion";
			}
		...
		public function InsertarEjemplar($datos){
			$query = "INSERT INTO libro(id_texto, id_edicion, id_editorial, isbn_libro) VALUES('$this->idTexto','$datos[edicion]','$datos[editorial]','$datos[isbn]')";
			$this->db->query($query);
			if($this->db->affected_rows < 0){
				echo "-ERROR: No se pudo insertar Libro";
			}
		}
