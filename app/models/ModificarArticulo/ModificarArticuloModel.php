<?php 
	...
	class ModificarArticuloModel extends Model
	{
		private $idAutor;
		private $idTexto;
		...
		public function MostrarDetalleArticulo($id){
			$query = "SELECT A.id_texto as id_texto, nombre_texto, fecha_publicacion, num_paginas, cantidad_total, cantidad_stock, nombre_genero, foto_portada FROM articulo A inner join texto T on T.id_texto=A.id_texto inner join coleccion C on T.id_texto=C.id_texto inner join genero G on G.id_genero=C.id_genero WHERE T.id_texto=$id and C.id_categoria=2";
			$res = $this->db->query($query);
			return $res;
		}
		...
		public function ActualizarArticulo($datos, $id){
